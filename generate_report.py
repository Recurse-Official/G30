...
def create_report(output_file, patient_data):
    ...
    title_style = ParagraphStyle(
        ...
        fontSize=18,
        textColor=colors.HexColor(dark_colour),
        alignment=1,
        spaceBefore=10,
        spaceAfter=10,
    )
    ...
    """
    contact_info = """
    Phone: +91 1234567890 | Email: omniflux@gmail.com
    """
    ...
    data = [
        ["Name", patient_data['name'], "Phone", patient_data['phone']],
        ["Email", patient_data['email'], "BMI", round(float(patient_data['weight'])/(float(patient_data['height'])/100)**2, 2)],
        ["Past Illness", patient_data['pastIllnesses'], "Present Medication", patient_data['currentMedication']]
    ]

    table = Table(data, colWidths=[1.5 * inch, 2 * inch, 1.5 * inch, 2 * inch])
    table.setStyle(TableStyle([
    ('BACKGROUND', (0, 0), (0, -1), colors.HexColor(light_colour)),
    ('BACKGROUND', (2, 0), (2, 2), colors.HexColor(light_colour)), 
    ...
    table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor(light_colour)),
        ('TEXTCOLOR', (0, 0), (-1, -1), colors.black),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ('FONTNAME', (0, 0), (-1, -1), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, -1), 10),
    ...
    contact_info = "Phone: +91 1234567890 | Email: OmniFlux@gmail.com"
...
def add_graphs(elements, patient_data):
    ...
    ax.bar(diseases, values, color='lightblue', width=bar_width)
    ...
    ax.pie(pie_values, labels=pie_labels, colors=['lightblue', 'darkblue', dark_colour], autopct='%1.1f%%', startangle=140)
...
def add_genomic_risk_score(elements, patient_data):
    ...
    for disease in patient_data['drug_recommendation']:
        ...
        fig, ax = plt.subplots()
        ax.barh(markers, risks, color='lightblue')
